...
export function createSkillsSection(
    ...
    render: ({ frame, theme, fontFamily }) => {
      ...
      const groups = skills
        .map((skill, index) => {
          ...
          const label = labels
            ? `<text x="${iconX + ICON_SIZE + 12}" y="${y + 44}" font-family="${fontFamily}" font-size="14" font-weight="600" fill="${theme.colors.foreground}">${escapeXml(skill.label)}</text>`
            : ''
  ...
  <text x="${CARD_PADDING}" y="${frame.y + 38}" font-family="${fontFamily}" font-size="${theme.typography.titleSize}" font-weight="700" fill="${theme.colors.foreground}">Skills</text>
${groups}
</g>`
    },
  })
