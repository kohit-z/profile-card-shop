...
export function renderAvatarEffect(
  ...
  <circle cx="${cx}" cy="${cy}" r="${radius + 18}" stroke="${theme.colors.accent}" stroke-opacity="0.3" stroke-dasharray="2 10">
    ...
    case 'equalizer': {
      const bars = Array.from({ length: 9 }, (_, index) => {
        ...
        const base = cy + radius + 36
        const first = 6 + (index % 4) * 3
        return `<rect x="${x}" y="${base - first}" width="6" height="${first}" rx="2" fill="${theme.colors.accent}">
  <animate attributeName="height" values="${first};${first + 14};${first + 4};${first}" dur="${1.05 + (index % 5) * 0.1}s" begin="${index * 0.08}s" repeatCount="indefinite" />
  <animate attributeName="y" values="${base - first};${base - first - 14};${base - first - 4};${base - first}" dur="${1.05 + (index % 5) * 0.1}s" begin="${index * 0.08}s" repeatCount="indefinite" />
</rect>`
      }).join('\n')
      return {
    ...
    case 'float':
      return {
        underlay: `<ellipse data-effect-layer="avatar" cx="${cx}" cy="${cy + radius + 19}" rx="38" ry="5" fill="${theme.colors.foreground}" opacity="0.14">
  <animate attributeName="rx" values="38;33;38;42;38" dur="5s" repeatCount="indefinite" />
</ellipse>`,
  ...
  <ellipse rx="${radius + 18}" ry="28" stroke-width="2" stroke-opacity="0.7" stroke-dasharray="34 18">
    <animateTransform attributeName="transform" type="rotate" from="0" to="360" dur="5s" repeatCount="indefinite" />
  </ellipse>
  <ellipse rx="${radius + 18}" ry="44" stroke-opacity="0.4" stroke-dasharray="18 14">
